  ...
  private static int tsp(int pos, int bitmask) { // bitmask stores the visited coordinates
    ...
    for (int nxt = 1; nxt <= n; nxt++){ // O(n) here
      if (nxt != pos && (bitmask & (1 << nxt)) == 0){ // if coordinate nxt is not visited yet
        ...
        if( dist[pos][nxt] < min_unvisited){  // Find the min cost edge and take that edge
          min_unvisited=dist[pos][nxt];
          min_pos=nxt;
        }
      }  
    }  
    ans=dist[pos][min_pos]+tsp(min_pos,bitmask|(1<<min_pos)); // greedy step , take the min cost edge 
